...
const VendorManagement = () => {
  ...
  const [form, setForm] = useState({
    vendor_name: '', gst_no: '', pan_no: '', mobile: '', alternate_mobile: '', 
    email: '', address: '', city: '', state: '', country: '', pincode: '', 
    opening_balance: 0, contact_person: '', notes: '', status: 'Active'
  });
  ...
  const fetchVendors = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${config.apiUrl}/vendors`, { headers: getAuthHeaders() });
      const data = await res.json();
      if (data.success) {
  ...
  const handleSearch = async (e) => {
    ...
    setSearch(value);
    try {
      const res = await fetch(`${config.apiUrl}/vendors?search=${value}`, { headers: getAuthHeaders() });
      const data = await res.json();
      if (data.success) {
        setVendors(data.data);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    setFormLoading(true);
    try {
      const url = editingId ? `${config.apiUrl}/vendors/${editingId}` : `${config.apiUrl}/vendors`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${config.apiUrl}/vendors/${id}`, {
        method: 'DELETE',
        headers: getAuthHeaders()
      });
      const data = await res.json();
  ...
  const exportCSV = () => {
    const headers = ['Code', 'Name', 'Mobile', 'Email', 'City', 'State', 'Status'];
    const csvData = vendors.map(v => [
      v.vendor_code, v.vendor_name, v.mobile, v.email, v.city, v.state, v.status
    ]);
    ...
    <div className="p-4 md:p-8 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">Manage suppliers and vendor ledgers</p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-gray-200 dark:border-slate-800 flex justify-between items-center bg-gray-50 dark:bg-slate-800/50">
          ...
          <button onClick={fetchVendors} className="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
              {vendors.map((v) => (
                <tr key={v._id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50">
                  <td className="px-6 py-4 font-mono font-medium text-blue-600">{v.vendor_code}</td>
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 text-xs font-semibold rounded-lg ${v.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                      {v.status}
                    </span>
                  </td>
                  {canManage && (
                    <td className="px-6 py-4 text-right space-x-2">
                      <button onClick={() => openEdit(v)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"><Edit2 size={16} /></button>
                      <button onClick={() => handleDelete(v._id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={16} /></button>
                    </td>
                  )}
                </tr>
              ))}
          ...
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-semibold mb-1.5">Vendor Name*</label>
                  <input required value={form.vendor_name} onChange={(e) => setForm({...form, vendor_name: e.target.value})} className="w-full px-4 py-2 border rounded-xl" />
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-1.5">Contact Person</label>
                  <input value={form.contact_person} onChange={(e) => setForm({...form, contact_person: e.target.value})} className="w-full px-4 py-2 border rounded-xl" />
                </div>
                <div>
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea value={form.address} onChange={(e) => setForm({...form, address: e.target.value})} className="w-full px-4 py-2 border rounded-xl" rows={2} />
                </div>
